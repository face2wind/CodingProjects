...
namespace face2wind {
  ...
  int ByteArray::ReadAllDataFromSocket(int socketID)
  {
    //std::cout<<"socket id  = "<<socketID<<std::endl;
    char buff[_readBuffSize];
    ////std::cout<<">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n";
    int readSize = 0;
    int flag = 0;
    bool firstRead = true;
    while(true){
      readSize = read(socketID, &buff, _readBuffSize);
      if(0 > readSize){
	//perror("read:");
	usleep(10);
	++flag;
	if(10 < flag) // 10 times no data, give up!
	  break;
	else
	  continue; // current cann't read, to next round
      }
      else if(0 == readSize){
	if(firstRead){ // no data on first read, means client was cut down
	  firstRead = false;
	  return 1;
	  //std::cout<<"socket disconnect on id = "<<socketID<<std::endl;
	  close(socketID);
	}
	break;
      }
	...
	////std::cout<<"buff["<<i<<"] = "//<<char(buff[i])<<"("
	//<<int((unsigned char)(buff[i]))<<")"<<std::endl;
	_bytes.push_back((unsigned char)buff[i]) ;
      }
    }
    ////std::cout<<"<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<\n";
    return 0;
  ...
  void ByteArray::WriteAllDataToSocket(int socketID)
  {
    int len = _bytes.size();
    char buff[len];
    for(int i = 0; i < len; i++)
      buff[i] = _bytes[i];
    if( send(socketID, buff, len, 0)< 0) {
      //std::cout<<"writeAllDataToSocket error: "<<strerror(errno)<<"(errno: %d)\n"<<errno;
      return;
    }
    _bytes.clear();
  }
